...
import { exportSchedule } from './export.js'; // IMPORTAMOS EXPORTADOR
...
let tooltipEl = null;
function initApp() {
    console.log("App v6.1 (Full Gestión + Export)");
    createTooltip();
    setupListeners();
...
function setupListeners() {
    ...
    bind('add-classroom-btn', addClassroom); // Botón de gestión de aulas
...
function renderScheduleGrid() {
    ...
    const fTch = document.getElementById('filter-teacher')?.value; const fGrp = document.getElementById('filter-group')?.value; const fTrim = document.getElementById('filter-trimester')?.value;
    ...
    state.blocks.forEach(b => {
        ...
        dIndices.forEach(di => {
            ...
            if(el.children[1]) el.children[1].style.pointerEvents = "auto"; frag.appendChild(el);
        });
    });
...
function showClassForm(defs = {}) {
    ...
    content.innerHTML = `
        <div class="p-6 bg-white rounded-lg"><h2 class="text-xl font-bold mb-4 text-gray-800">${defs.id ? 'Editar' : 'Nueva'} Clase</h2>
            ...
            </div><div class="flex justify-end gap-3 mt-6"><button id="btn-cancel" class="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded">Cancelar</button><button id="btn-save" class="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 shadow">Guardar</button></div></div>`;
    ...
    document.getElementById('btn-save').onclick = async () => {
        ...
        if (conflicts.length > 0) {
            ...
            document.getElementById('btn-force').onclick = async () => { await commitSave(payload, defs.id); }; return;
        }
...
function showTeacherForm(teacher = null) {
    ...
    document.getElementById('btn-t-save').onclick = async () => { const n = document.getElementById('t-name').value; const f = document.getElementById('t-full').value; if(n) { const data = {name: n, fullName: f}; if(isEdit) await updateDoc(doc(cols.teachers, teacher.id), data); else await addDoc(cols.teachers, data); modal.classList.add('hidden'); } };
}
...
async function handleDrop(e, day, hour) {
    ...
    try { const d = JSON.parse(e.dataTransfer.getData('application/json')); if(d.type === 'subject') { const s = state.subjects.find(x => x.id === d.id); showClassForm({day, startTime: hour, subjectId: d.id, teacherId: s ? s.defaultTeacherId : null}); } } catch(err){}
}
function validateConflicts(newClass, ignoreId) {
    ...
    state.schedule.forEach(e => {
        ...
        if(ns < ee && ne > es) { if(e.teacherId === newClass.teacherId) conflicts.push("El docente ya tiene clase"); if(e.groupId === newClass.groupId) conflicts.push("El grupo ya tiene clase"); if(newClass.classroomId && e.classroomId === newClass.classroomId) conflicts.push("El aula está ocupada"); }
    });
    return conflicts;
}
...
function renderFilterOptions() {
    ...
    // FIX: Agregar explicitamente la opción "Todos"
...
function renderSubjectsList() {
    const c = document.getElementById('unassigned-subjects-container'); if(c) {
        c.innerHTML = '';
    ...
    const l2 = document.getElementById('subjects-by-trimester'); if(l2) {
        ...
        const grouped = {}; state.subjects.forEach(s => { const t = s.trimester||0; if(!grouped[t]) grouped[t]=[]; grouped[t].push(s); });
        Object.keys(grouped).sort((a,b)=>Number(a)-Number(b)).forEach(t => {
            const items = grouped[t].map(s => `<div class="flex justify-between items-center p-1 border-b last:border-0 border-gray-100 hover:bg-white text-xs"><span class="truncate" title="${s.name}">${s.name}</span><button onclick="window.editSub('${s.id}')" class="text-blue-400 px-1">✎</button></div>`).join('');
