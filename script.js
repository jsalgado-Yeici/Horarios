...
import { renderMap, MAP_DATA } from './maps.js'; // Importamos MAP_DATA también
...
let tooltipEl = null;
function initApp() {
    console.log("App v6.0 (Map Sync & Details)");
    createTooltip();
    setupListeners();
...
// === LÓGICA DE SINCRONIZACIÓN AUTOMÁTICA DE AULAS ===
async function syncMapRooms() {
    // Solo ejecutamos si ya tenemos las aulas cargadas de Firebase
    if(state.loading.classrooms) return;

    const mapRooms = [...MAP_DATA.pa, ...MAP_DATA.pb];
    let addedCount = 0;

    for (const room of mapRooms) {
        // Filtramos: Solo sincronizar si es classroom, lab o los específicos que pediste aunque estén desactivados
        // Si quieres que 'baile' y 'cert' aparezcan en la lista aunque sean 'office' en el mapa visual:
        const shouldSync = room.type === 'classroom' || room.type === 'lab' || room.id === 'baile' || room.id === 'cert';
        
        if (shouldSync) {
            // Verificar si ya existe en la BD (por ID del mapa o por nombre exacto)
            const exists = state.classrooms.some(c => c.id === room.id || c.name === room.name);
            
            if (!exists) {
                console.log(`Auto-agregando aula: ${room.name}`);
                // Usamos setDoc con el ID del mapa para evitar duplicados futuros
                try {
                    await setDoc(doc(cols.classrooms, room.id), {
                        name: room.name,
                        type: room.type,
                        autoGenerated: true
                    });
                    addedCount++;
                } catch(e) { console.error("Error sync room:", e); }
            }
        }
    }
    if(addedCount > 0) console.log(`${addedCount} aulas sincronizadas.`);
}

// === MAPA CON DETALLES REALES ===
...
function showRoomDetails(room) {
    ...
    let html = `
        <div class="p-6 bg-white">
            <h2 class="text-2xl font-bold mb-1 text-gray-800">${room.name}</h2>
            <p class="text-sm text-gray-500 mb-4">ID Sistema: ${room.id}</p>
            
            <div class="border-t border-gray-100 pt-4 max-h-[60vh] overflow-y-auto">
    `;

    if(classes.length === 0) {
        html += `<div class="text-center py-8 text-gray-400 italic">No hay clases asignadas a este salón.</div>`;
    } else {
        html += `<table class="w-full text-sm text-left">
                    <thead class="text-xs text-gray-500 uppercase bg-gray-50">
                        <tr><th class="px-2 py-2">Día</th><th class="px-2 py-2">Hora</th><th class="px-2 py-2">Materia</th><th class="px-2 py-2">Docente</th></tr>
                    </thead>
        ...
        classes.forEach(c => {
            ...
            const color = state.subjects.find(s => s.id === c.subjectId) ? PALETTE[c.subjectId.charCodeAt(0)%PALETTE.length] : '#ccc';
            
            html += `
                <tr class="hover:bg-gray-50">
                    <td class="px-2 py-3 font-medium text-gray-700">${c.day}</td>
                    <td class="px-2 py-3">${c.startTime}:00 - ${c.startTime + c.duration}:00</td>
                    <td class="px-2 py-3 font-semibold" style="color:${color}">${subj}</td>
                    <td class="px-2 py-3 text-gray-500">${teach}</td>
                </tr>
            `;
        });
        html += `</tbody></table>`;
    ...
    html += `</div>
             <div class="mt-6 flex justify-end">
                <button onclick="document.getElementById('modal').classList.add('hidden')" class="px-4 py-2 bg-gray-800 text-white rounded hover:bg-black transition-colors">Cerrar</button>
             </div>
        </div>`;
...
function setupRealtimeListeners() {
    const update = (k, s) => {
        ...
        // SYNC TRIGGER: Cuando carguen las aulas, sincronizar mapa
        if(k === 'classrooms') {
            renderClassroomsList();
            syncMapRooms(); // <--- AQUÍ OCURRE LA MAGIA
        }
    };
...
// RENDER GRID (Mantener igual que antes)
...
// FORMS & HELPERS (Sin cambios, solo para completar el archivo)
...
function renderClassroomsList() { const l = document.getElementById('classrooms-list'); if(l) l.innerHTML = state.classrooms.map(c => `<div class="p-1 border-b text-xs flex justify-between">${c.name} <button class="text-red-500" onclick="delDoc('classrooms','${c.id}')">x</button></div>`).join(''); }
