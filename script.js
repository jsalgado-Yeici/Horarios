...
let tooltipEl = null;
function initApp() {
    console.log("App v5.2 (Drag Fix + Todos Cuatris)");
    createTooltip();
    setupListeners();
...
function setupRealtimeListeners() {
    const update = (k, s) => {
        ...
        // Renderizar filtros y listas cuando cambian los datos
...
function renderScheduleGrid() {
    ...
    const visible = state.schedule.filter(c => {
        ...
        // Si fTrim está vacío (""), muestra todo. Si tiene valor, filtra.
        if(fTrim) { 
...
async function handleDrop(e, day, hour) {
    ...
    } catch(err){ console.error("Error al soltar:", err); }
}
...
function renderFilterOptions() {
    ...
    // FIX PARA "TODOS LOS CUATRIS"
    const t = document.getElementById('filter-trimester'); 
    if(t) {
        const v = t.value;
        t.innerHTML = '<option value="">Todos los Cuatris</option>'; // Opción Default
        for(let i=1; i<=9; i++) t.add(new Option(`Cuatrimestre ${i}`, i));
        t.value = v;
    }
}
    ...
    state.teachers.sort((a,b)=>a.name.localeCompare(b.name)).forEach(t => {
        ...
        l.innerHTML += `
            <div class="p-2 border-b text-sm flex justify-between items-center bg-white hover:bg-gray-50">
                <div class="leading-tight"><span class="font-bold text-gray-700 cursor-pointer" onclick="window.editTeacher('${t.id}')">${t.name}</span>${full}</div>
                <div class="flex gap-1"><button class="text-blue-400 px-1" onclick="window.editTeacher('${t.id}')">✎</button><button class="text-red-400 px-1" onclick="delDoc('teachers','${t.id}')">×</button></div>
            </div>`;
    });
}
    ...
    if(c) {
        ...
        Object.keys(grouped).sort((a,b)=>Number(a)-Number(b)).forEach(t => {
            ...
            grouped[t].forEach(s => {
                ...
                // FIX: USAR DATASET O LISTENER PARA EVITAR COMILLAS ROTAS
