...
// === LISTA DOCENTES (BARRA DE PROGRESO REAL VS META + ASESORÍAS) ===
export function renderTeachersList() { 
    ...
    state.teachers.sort((a,b)=>a.name.localeCompare(b.name)).forEach(t => { 
        // 1. Horas CLASE Programadas (En el grid, excluyendo asesorías)
        ...
        // 3. META de Asesoría (Definida en perfil del docente)
        ...
        // Barra de progreso (Base 40h o ajustable)
        ...
        // Indicador visual de cumplimiento de asesorías
        const advStatusColor = advisoryScheduled >= advisoryGoal ? "text-green-600" : "text-red-500";
        const advText = advisoryGoal > 0 
            ? `<span class="text-[9px] ${advStatusColor} font-bold ml-1" title="Asesorías: Programadas ${advisoryScheduled} / Meta ${advisoryGoal}">(Ases. ${advisoryScheduled}/${advisoryGoal}h)</span>` 
            : '';
            ...
            <div class="flex justify-between items-center">
                ...
                <div class="flex gap-1 items-center">
                    <div class="flex flex-col items-end leading-none mr-2">
                        <span class="text-xs font-mono font-bold text-gray-700" title="${breakdownTitle}">${totalScheduled}h Totales</span>
                        ${advText}
                    </div>
                    <button class="btn-edit text-blue-400 px-1 hover:text-blue-600">✎</button>
                    <button class="btn-del text-red-400 px-1 hover:text-red-600">×</button>
                </div>
            </div>
...
export function renderGroupsList() { 
    ...
    l.querySelectorAll('.group-item').forEach(el => {
        ...
        el.querySelector('.btn-edit-grp').onclick = () => showGroupForm(g);
        el.querySelector('.btn-del-grp').onclick = () => deleteDocWrapper('groups', id);
    });
}

// === LISTA AULAS (CON EDITAR/BORRAR) ===
export function renderClassroomsManageList() { 
    ...
    l.innerHTML = state.classrooms.map(c => `
        <div class="p-1 border-b text-xs flex justify-between items-center classroom-item" data-id="${c.id}">
            <span>${c.name}</span> 
            <div class="flex gap-1">
                <button class="text-blue-400 hover:text-blue-600 btn-edit-room">✎</button>
                <button class="text-red-500 hover:text-red-700 btn-del-room">x</button>
            </div>
        </div>`
    ).join('');
    ...
    l.querySelectorAll('.classroom-item').forEach(el => {
        ...
        el.querySelector('.btn-edit-room').onclick = () => showClassroomForm(c);
        el.querySelector('.btn-del-room').onclick = () => deleteDocWrapper('classrooms', id);
    });
}

// === AUDITORÍA INTELIGENTE (CHIPS COMPACTOS) ===
export function renderAlerts() {
    ...
    state.groups.forEach(g => {
        ...
        // Excluir asesorías del conteo de materias académicas
        const assigned = state.schedule
            .filter(c => c.groupId === g.id && c.type !== 'advisory')
            .map(c => c.subjectId);
...
// === MODAL DE SUGERENCIAS INTELIGENTES (IA) ===
...
// Función global para aplicar sugerencia desde el HTML string
window.applySuggestion = async (subId, grpId, teachId, roomId, day, start, dur) => {
    try {
        const payload = {
            type: 'class', // Forzamos tipo clase
            subjectId: subId, 
            groupId: grpId, 
...
function findSmartSlots(subject, group) {
    ...
    const duration = 2; // Duración default para sugerencias

    for (const d of days) {
        for (const h of timeSlots) {
            if (h + duration > Math.max(...timeSlots) + 1) continue; 

            // 1. Verificar GRUPO libre
            const groupBusy = state.schedule.some(c => 
                c.groupId === group.id && c.day === d && 
                !(c.startTime >= h + duration || c.startTime + c.duration <= h)
            );
            
            // Verificar BLOQUEOS EXTERNOS del grupo
            const extBusy = state.external.some(e => 
                e.groupId === group.id && e.day === d && 
                !(e.start >= h + duration || e.end <= h)
            );
            
            if (!groupBusy && !extBusy) {
                // 2. Verificar DOCENTE libre (si existe)
                let teacherFree = true;
                if (teacherId) {
                    teacherFree = !state.schedule.some(c => 
                        c.teacherId === teacherId && c.day === d && 
                        !(c.startTime >= h + duration || c.startTime + c.duration <= h)
                    );
                }

                if (teacherFree) {
                    // 3. Buscar cualquier AULA libre
                    const freeRoom = state.classrooms.find(r => {
                         return !state.schedule.some(c => 
                            c.classroomId === r.id && c.day === d && 
                            !(c.startTime >= h + duration || c.startTime + c.duration <= h)
                         );
                    });
...
export function renderFilterOptions() { 
    ...
    // Lógica dinámica para Cuatrimestres basada en Turno
    ...
    [...state.groups].sort((a,b) => a.name.localeCompare(b.name)).forEach(grp => {
        ...
        timeSlots.forEach(t => {
            ...
            const cls = state.schedule.find(c => 
                c.groupId === grp.id && c.day === dayFilter && 
                c.startTime <= t && (c.startTime + c.duration) > t &&
                c.type !== 'advisory' // Excluir asesorías en sábana de grupos (pues son del profe)
            );
            
            // Buscar Externa
            const ext = state.external.find(e => 
                e.groupId === grp.id && e.day === dayFilter && 
                e.start <= t && e.end > t
            );
